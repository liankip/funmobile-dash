<template>
	<table>
		...
		<tbody>
			<tr v-for="(count, index) in countTrxf.result" :key="index">
...
export default {
	...
	computed: {
		...mapGetters(['countTrxf']),
	},

	methods: {
		...mapActions(['fetchTrxf']),
		currency,

		retrieveOrderf() {
			this.fetchTrxf({ page: this.page, status: this.status });
		},
